...
function checkClientAuth(schema) {
  return !!clientAuthEndpoints.find((endpoint) =>
    ["private_key_jwt", "self_signed_tls_client_auth"].includes(
      schema[`${endpoint}_endpoint_auth_method`]
    )
  );
}
  ...
  const ENUM = {
    ...
    backchannel_authentication_request_signing_alg: () =>
      configuration.requestObjectSigningAlgValues.filter(
        (alg) => alg !== "none" && !alg.startsWith("HS")
      ),
    ...
    introspection_endpoint_auth_signing_alg: ({ introspection_endpoint_auth_method: method }) => {
      switch (method) {
        case "private_key_jwt":
          return configuration.introspectionEndpointAuthSigningAlgValues.filter(
            (x) => !x.startsWith("HS")
          );
        case "client_secret_jwt":
          return configuration.introspectionEndpointAuthSigningAlgValues.filter((x) =>
            x.startsWith("HS")
          );
        default:
          return [];
    ...
    revocation_endpoint_auth_signing_alg: ({ revocation_endpoint_auth_method: method }) => {
      switch (method) {
        case "private_key_jwt":
          return configuration.revocationEndpointAuthSigningAlgValues.filter(
            (x) => !x.startsWith("HS")
          );
        case "client_secret_jwt":
          return configuration.revocationEndpointAuthSigningAlgValues.filter((x) =>
            x.startsWith("HS")
          );
        default:
          return [];
  ...
  class Schema {
    constructor(
      ...
      ["revocation", "introspection"].forEach((endpoint) => {
        if (metadata[`${endpoint}_endpoint_auth_method`] === undefined) {
          Object.assign(metadata, {
            [`${endpoint}_endpoint_auth_method`]:
              metadata.token_endpoint_auth_method ||
              configuration.clientDefaults.token_endpoint_auth_method,
          });
        }
        if (
          metadata[`${endpoint}_endpoint_auth_signing_alg`] === undefined &&
...

      {
        ...
        let used;
        for (const endpoint of clientAuthEndpoints) {
          ...
          case "web": {
            ...
            if (this.grant_types.includes("implicit")) {
              ...
              if (hostname === "localhost") {
                this.invalidate(
                  "redirect_uris for web clients using implicit flow must not be using localhost",
          ...
          case "native": {
            switch (protocol) {
              ...
              case "https:": // Claimed HTTPS URI Redirection
                if (LOOPBACKS.has(hostname)) {
                  this.invalidate(
                    `redirect_uris for native clients using claimed HTTPS URIs must not be using ${hostname} as hostname`
                  );
                }
                break;
    ...
    jarPolicy() {
      const {
        features: { requestObjects, pushedAuthorizationRequests },
      } = configuration;

      const enabled =
        requestObjects.request || requestObjects.requestUri || pushedAuthorizationRequests.enabled;
      ...
      if (enabled) {
        ...
        if (this.require_signed_request_object && this.request_object_signing_alg === "none") {
          this.invalidate(
            'request_object_signing_alg must not be "none" when require_signed_request_object is true'
          );
        }
      }
    }
    ...
    backchannelLogoutNeedsIdTokenAlg() {
      if (this.backchannel_logout_uri && this.id_token_signed_response_alg === "none") {
        this.invalidate(
          'id_token_signed_response_alg must not be "none" when backchannel_logout_uri is used'
        );
      }
    }
